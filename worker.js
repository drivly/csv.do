...
export default {
  fetch: async (req, env) => {
    const { user, origin, requestId, method, body, time, pathname, search, pathSegments, pathOptions, query } = await env.CTX.fetch(req).then(res => res.json())
    let { delimiter = ',', fields } = pathOptions
    const url = `https://${pathOptions ?  pathSegments.slice(1).join('/') : pathSegments.join('/')}${search}`
